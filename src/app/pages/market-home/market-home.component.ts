...
export class MarketHomeComponent implements OnInit, OnDestroy {
    ...
    subs: Subscription[] = [];
    data: any[] = [];
    allProducts$: Observable<Product[]> = this.store.select(
    ...
    ngOnInit(): void {
        this.subs.push(
            this.allProducts$.subscribe((allProducts: Product[]) => {
                this.data = this.splitArray(allProducts);
            })
        );
        this.store.dispatch(
            appStore.modules.productsModule.actions.fetchProductList()
        );
    }
